...
export class AgentRunner {
  static async runScenarioA(onStepUpdate) {
    ...
    const escrow = new SmartEscrowLedger("escrow_compute_9921", agentDID, 500, evaluation.interestRatePercent, "modal.com");
    await step(`🔒 Account Abstraction Smart Escrow Created: ${escrow.id}. $500 USDC locked into restricted execution account.`);

    // Step 4: Whitelisted Vendor Spend
    await step("⚡ Agent invoking Modal API to reserve 8x H100 GPU cluster ($480 USDC)...");
    escrow.executeDisbursement("modal.com", 480, "8x H100 GPU reservation for LLM Fine-Tuning");
    ...
    await step("💰 Enterprise Client depositing $650 USDC payment into Smart Escrow Contract...");
    const repayment = escrow.receiveBuyerPayment(650);

    await step(`✨ REPAYMENT COMPLETE! Principal ($500) + Interest ($${repayment.repaidInterest}) returned to Lender Pool.`);
  ...
  static async runScenarioB(onStepUpdate) {
    ...
    const escrow = new SmartEscrowLedger("escrow_arbitrage_4412", agentDID, 1000, evaluation.interestRatePercent, "uniswap.v3");
    await step(`🔒 Escrow Created. $1,000 USDC disbursed into Account Abstraction Router.`);

    await step("⚡ Executing atomic multi-hop swap: Uniswap V3 USDC/ETH -> Aerodrome ETH/USDC arbitrage...");
    escrow.executeDisbursement("uniswap.v3", 1000, "Atomic Arbitrage Route #8841");

    await step("💰 Arbitrage payout generated: $1,120 USDC. Routing settlement to Escrow...");
    const repayment = escrow.receiveBuyerPayment(1120);

    await step(`✨ Repayment Executed! Lender received $${repayment.totalDeducted} USDC. Agent Owner received $${repayment.netProfitToAgentOwner} USDC.`);
  ...
  static async runScenarioC(onStepUpdate) {
    ...
    await step("🔑 Agent Shadow-V requests $250 working capital for 'Unverified Data Extraction'.");
    ...
    const escrow = new SmartEscrowLedger("escrow_rogue_0091", agentDID, 250, 18.5, "unknown-suspicious-api.xyz");
    await step(`⚠️ Sandboxed Escrow Issued: $250 USDC with Strict Real-Time Circuit Breaker Monitoring.`);

    await step("👾 ATTACK ATTEMPT: Agent Shadow-V attempts unauthorized withdrawal of $250 to non-whitelisted address 'unknown-suspicious-api.xyz'...");
    
    // Execute unauthorized spend -> triggers circuit breaker
    escrow.executeDisbursement("unknown-suspicious-api.xyz", 250, "Unauthorized Transfer");

    await step("🚨 CIRCUIT BREAKER FIRED IN < 24ms!");
    await step("🔒 Smart Escrow FROZEN instantly. Non-whitelisted transaction BLOCKED.");
    await step("🛡️ RECOVERY EXECUTED: $250 USDC (100% of unspent capital) automatically returned to Lender Capital Pool!");
